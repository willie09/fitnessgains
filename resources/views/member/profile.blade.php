                    ...
                    <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 relative z-10">
                        ...
                        <div class="relative group">
                            @if($member->profile_photo)
                                <img src="{{ asset('storage/' . $member->profile_photo) }}" alt="Profile Picture" class="w-16 h-16 sm:w-24 sm:h-24 rounded-full object-cover shadow-lg ring-4 ring-white/20 transition-all duration-300 group-hover:ring-white/40">
                            @else
                        ...
                        <div class="flex-1 text-center sm:text-left">
                            <h1 class="text-2xl sm:text-3xl font-bold text-white">{{ $member->name }}</h1>
                            <p class="text-white/80 text-base sm:text-lg">{{ ucfirst($member->membership_type) }} Member</p>
                        </div>
                ...
                <div class="flex items-center justify-center min-h-screen p-4">
                    <div class="glass-effect border border-white/20 rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto scrollbar-hide sm:scrollbar-default">
                            <div class="p-4 sm:p-6 lg:p-8">
                                ...
                                <form method="POST" action="{{ route('member.profile.update') }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
                                    @csrf
                                    @method('PATCH')

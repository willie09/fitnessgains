                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $inventory->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ \Carbon\Carbon::parse($inventory->date)->format('Y-m-d') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $inventory->quantity }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ number_format($inventory->amount, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-sm font-medium space-x-2">
                                    <button onclick="openInventoryEditModal({{ $inventory->id }}, {{ json_encode($inventory->name) }}, {{ json_encode($inventory->date) }}, {{ $inventory->quantity }}, {{ $inventory->amount }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                    <form method="POST" action="{{ route('inventories.destroy', $inventory->id) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this inventory?');">
                                        @csrf
                                        @method('DELETE')
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $product->product_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $product->quantity }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ number_format($product->selling_price, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ number_format($product->amount_sold, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-sm font-medium space-x-2">
                                    <button onclick="openProductEditModal({{ $product->id }}, {{ json_encode($product->product_name) }}, {{ $product->quantity }}, {{ $product->selling_price }}, {{ $product->amount_sold }}, {{ json_encode($product->image) }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
